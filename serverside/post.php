            ...
            if (!empty($result)) {
                if (strcasecmp($result[0]["email"], $email) == 0) {
                    ...
                    foreach ($result as $row) {
                        ...
                        $_SESSION['islogin'] = 1;
                        $Functions->setlastSeen($_SESSION['user_id']);
                        echo "true";
                    }
                } else {
                    echo "false";
...
//recover password
    ...
    $post_code  = htmlspecialchars(stripslashes($_POST['post_code']));
    $post_code  = $db->escapeString($post_code);
    $lat_long = $func->getLatLong($post_code);
    ...
    (user_id,title,post_code,main_type,sub_type,options,job_discription,created_date,`country`, `city`, `location`, `latitude`, `longitude`)values 
    ('$user_id','$title','$post_code','$main_type','$sub_type','$options','$note','$later','$country', '$city', '$location','$latitude', '$longitude')";
        ...
        if ($db->sql($sql)) {
            ...
            if ($db->sql($sql)) {
                ...
                if($result){
                    ...
                    $user_id = $result['id'];
                    $sql = "SELECT token from fcm_token WHERE user_id = '$user_id'";
...
//Apply for Job
    ...
    $stars_q2 = htmlspecialchars(stripslashes($_POST['stars_q2']));
    $stars_q2 = $db->escapeString($stars_q2);

    $stars_q3 = htmlspecialchars(stripslashes($_POST['stars_q3']));
    $stars_q3 = $db->escapeString($stars_q3);
    ...
    $sql = "INSERT INTO rateuser (from_user_id, user_id, job_id, job_title, ratings, ratings_q2, ratings_q3, recommendation, message, user_info)
        VALUES ('$from_user_id', '$user_id', '$job_id', '$job_title', '$stars', '$stars_q2', '$stars_q3', '$recommedns', '$message', '$escapedUserInfo')";
    if ($db->sql($sql)) {
...
//update settings
    ...
    if ($subsData['status'] == 'active') {
        ...
        //send push notification if have refer
        if (($user_info[0]['from_referral_code'] != "")){
            ######## NOTIFICATION START ########
            $refer_code = $user_info[0]['from_referral_code'];
            $result = $conn->query("SELECT users.id FROM users WHERE users.to_referral_code = '$refer_code' LIMIT 1")->fetch_assoc();

            $notify_user_tokens = [];
            $username = $user_info[0]['fname'];
            if($result){
            ...
            $title = "New referral";
            $body = "You have a new referral from ".$username;
...
// update intoduction
    ...
    if ($db->sql($sql)) {
        ...
        if ($db->sql($sql)) {
            ...
            $sql = "SELECT users.id,users.user_role,users.post_code,users.status,users.create_date, verify_skill.main_category, verify_skill.status , verify_skill.verify
            FROM users
                    ...
                    foreach ($result as $row) {
                        ...
                        if ($db->sql($sql)) {
                            ...
                            foreach ($tokens as $token) {
                                array_push($notify_user_tokens, $token['token']);
                            }
                        }
                    }
                }
...
// Sending chat messages
    ...
    if ($db->sql($sql)) {
        ...
        $sql = "SELECT token FROM fcm_token WHERE user_id = '$touserid'";
        $db->sql($sql);
        $res = $db->getResult();
        $notify_user_tokens = [];
        if (count($res) > 0) {
            foreach ($res as $data) {
                $notify_user_tokens[] = $data['token'];
            }
        }
...
?>
    ...
    //Reward to homeowner from users table
        ...
        if ($db->sql($sql)) {
             ######## NOTIFICATION START ########
             $title = "Congratulations, you've won one of our amazing rewards";
             $body =  "Congratulations, you've won one of our amazing rewards";
    ...
    //send new blog notification
        ...
        if ($db->sql($sql)) {
            ...
            ######## NOTIFICATION START ########
            $title = "Great news " . $user['fname'] . ", your account has now been verified";
            $body =  "Great news " . $user['fname'] . ", your account has now been verified";
