                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    foreach($mySkills as $skill){
                                        ...
                                        ?>
                                        <span><?=$main_name[0]['category_name']?>,</span>
                                        <?php
                                    }
                                    ?>
                                ...
                                ?>
                                <td class="action">
                                    <a class="btn btn-primary m-1" href="../user-profile?u_id=<?=$user['id']?>">View</a>

                                    <?php

                                    if($user['status']==1){
                                        ?>
                                        <a class="btn btn-danger m-1" onclick="blockUser(this.id)" id="<?=$user['id']?>">Block</a>
                                        <?php
                                    }else{
                                        ?>
                                        <a class="btn btn-success m-1" onclick="activeUser(this.id)" id="<?=$user['id']?>">Activate</a>
                                        <?php
                                    }

                                    ?>
                                    <a class="btn btn-danger m-1" onclick="deleteUser(this.id)" id="<?=$user['id']?>" >Delete</a>
                                    <?php
                                    if(count($account)>0){
                                        ?>
                                        <button class="m-1 btn btn-success" onclick="showUserAccount(<?=$user['id']?>)" >View bank details</button>

                                        <?php
                                    }
                                    ?>
                                    <a class="btn btn-info m-1 reward" data-user_type="<?=$user['user_role']?>" data-target="#givereward" data-toggle="modal"  data-userid="<?=$user['id']?>" data-userfname="<?=$user['fname']?>" data-userlname="<?=$user['lname']?>" data-usertown="<?=$user['town']?>" data-useremail="<?=$user['email']?>" onClick="
                                    showDiv('<?=$user['user_role']?>','<?=$user['id']?>','<?=$user['fname']?>','<?=$user['lname']?>','<?=$user['town']?>','<?=$user['email']?>');
                                    " id="<?="user_reward_id_".$user['id']?>">Give Reward</a>
