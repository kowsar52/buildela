...
function fetchJobs($conn, $userId, $type, $page, $limit) {
    ...
    if (!empty($categories_ids)) {
        ...
        $totalJobsResult = $conn->query("SELECT FOUND_ROWS() as total_jobs;");
        ...
        $sql =  "SELECT post_job.*, 
            ...
            add_options.option,
            users.fname AS postby_fname, users.lname AS postby_lname, users.post_code AS postby_post_code, users.phone AS postby_phone,
            jobs_gallery.file_type AS gallery_file_type, jobs_gallery.img_path AS gallery_img_path
        FROM post_job 
        ...
        LEFT JOIN jobs_gallery ON post_job.id = jobs_gallery.job_id 
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                // $func = new Functions();
                // $row['distance'] =  $func->batchDistance($row['postby_post_code'], $row['post_code'], 'mile');
                if($type == 'all' || $type == 'interseted' || $type == 'new_leads'){
                    $row['postby_phone'] = Null;
                }
                $row['job_status'] =  $row['status'] == '1' ? 'Active':'Inactive';
    ...
    // // Step 1: Get the job IDs based on the status and user ID
    // switch ($status) {
    //     case 0:
    //         if (!empty($categories_ids)) {
    //             $sql = "SELECT post_job.id AS job_id 
    //                     FROM post_job 
    //                     LEFT JOIN apply_job ON post_job.id = apply_job.job_id AND apply_job.user_id = ?
    //                     WHERE apply_job.id IS NULL AND post_job.main_type IN ($categories_ids)"; 
    //         } else {
    //             $sql = "SELECT post_job.id AS job_id 
    //                     FROM post_job 
    //                     LEFT JOIN apply_job ON post_job.id = apply_job.job_id AND apply_job.user_id = ?
    //                     WHERE apply_job.id IS NULL";
    //         }
    //         break;
    ...
    //         break;
    //     case 2:
    //         $sql = "SELECT job_id FROM apply_job WHERE status='1' AND worker_status = '0' AND employer_status = '0' AND user_id=?";
    //         break;
    //     case 3:
    //         $sql = "SELECT job_id FROM apply_job 
    //                 WHERE user_id=?
    //                 AND (
    //                     (status='1' AND worker_status = '0' AND employer_status = '1') OR
    //                     (status='1' AND worker_status = '1' AND employer_status = '1') OR
    //                     (status='2' AND worker_status = '1' AND employer_status = '1')
    //                 )";
    //         break;

    //     default:
    //         $sql = "SELECT job_id FROM apply_job WHERE user_id=?";
    //         break;
    // }
    ...
    // // For case 1, after getting the applied jobs, get the actual jobs from `post_job`
    // // Fetch the job details using JOINs (for both case 1 and 2)
    // if ($status == 1 || $status == 2) {
    //     $sql = "SELECT post_job.*, 
    //         main_category.category_name, 
    //         sub_category.category_name AS sub_category_name, 
    //         add_options.option,
    //         users.fname AS postby_fname, users.lname AS postby_lname, users.post_code AS user_post_code
    //         CONCAT('[', 
    //             GROUP_CONCAT(
    //                 JSON_OBJECT(
    //                     'job_id', jobs_gallery.job_id,
    //                     'file_type', jobs_gallery.file_type,
    //                     'img_path', jobs_gallery.img_path
    //                 )
    //                 SEPARATOR ','
    //             ),
    //             ']'
    ...
    //     LEFT JOIN add_options ON post_job.options = add_options.id 
    //     LEFT JOIN jobs_gallery ON post_job.id = jobs_gallery.job_id 
    //     WHERE post_job.id IN ($jobIds) AND post_job.user_id != ? 
    //     ORDER BY post_job.id DESC
    //     LIMIT ? OFFSET ?";
    ...
    // } else {
    //     $sql = "SELECT SQL_CALC_FOUND_ROWS post_job.*, main_category.category_name, 
    //         sub_category.category_name as sub_category_name, add_options.option, 
    //         CONCAT('[', 
    //             GROUP_CONCAT(
    //                 JSON_OBJECT(
    //                     'job_id', jobs_gallery.job_id,
    ...
    //             ),
    //             ']'
    //         ) AS job_gallery, users.fname AS postby_fname, users.lname AS postby_lname, users.post_code AS user_post_code
    //     FROM post_job
    //     LEFT JOIN main_category ON post_job.main_type = main_category.id
    //     LEFT JOIN sub_category ON post_job.sub_type = sub_category.id
    //     LEFT JOIN add_options ON post_job.options = add_options.id
    ...
    //         $func = new Functions();
    //         $row['distance'] =  $func->batchDistance($row['user_post_code'], $row['post_code'], 'mile');
    //         $row['job_status'] =  $row['status'] == '1' ? 'Active':'Inactive';
...
function blogsPercat($conn, $identity = 'trade', $count=5) {
    // pulling categories
    if($identity == 'trade') $sql = "SELECT * FROM blog_category WHERE cat_type='tradespeople_category'";
    elseif($identity == 'home') $sql = "SELECT * FROM blog_category WHERE cat_type='home_owners_category'";
    ...
    // pulling blogs
    if($identity == 'trade') $bsql = "SELECT * FROM blogs WHERE category='Professionals' ORDER BY RAND()";
    elseif($identity == 'home') $bsql = "SELECT * FROM blogs WHERE category='Homeowners' ORDER BY RAND()";
...
function getallBlogsbyCat($conn, $cats, $identity = 'trade', $page) {
    ...
    if($identity == 'trade') $sql = "SELECT * FROM blogs WHERE category = 'Professionals' AND blog_category $inClause ORDER BY create_date DESC LIMIT ? OFFSET ?";
    if($identity == 'home') $sql = "SELECT * FROM blogs WHERE category = 'Homeowners' AND blog_category $inClause ORDER BY create_date DESC LIMIT ? OFFSET ?";
...
function postJobWithMedia($conn, $data) {
  ...
  $sql="insert into post_job 
    ...
    if ($conn->query($sql)) {
        ...
        echo json_encode(array("message" => "Failed to post job. Error: " . $conn->error), JSON_PRETTY_PRINT);
    }
}
