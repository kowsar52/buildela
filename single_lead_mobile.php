...
$countjobs          =   0;
$distance           =   $_POST['distance'];
$jobs               =   $func->getSingleJob($_POST['job_id']);
...
$jobImages          =   $func->getJobsImages($_POST['job_id']);
$applyJob           =   $func->getSingleApplyJob($_POST['job_id']);
$totalAppliedUser   =   $func->getAllApplyUser($_POST['job_id']);
$shortListedUser    =   $func->shortlistedusers($_POST['job_id']);
$checkStatus        =   $func->getApplyUserStatus($_SESSION['user_id'],$_POST['job_id']);
$usersStatus        =   $func->getjobposteduserinfo($_POST['job_id']);
$new_chat           =   $func->getAllNewChates($jobs[0]['user_id'],$jobs[0]['id']);
$check_chat_exist   =   $func->isChatexist($jobs[0]['id']);
$newDescription     =   $func->getNewDescriptionOfJob($_POST['job_id']);
...
// if($func->isMobileDevice()){
//     if(!$func->isLeadRead($_POST['job_id'], )){
//         $func->setLeadRead($_POST['job_id']);
//     }
//     if($reading === 'unread'){
...
$applyJobnew=$func->getAllApplyUser_new($_POST['job_id']);
if((!empty($applyJobnew) )&&($applyJobnew[0]['worker_status']==1)){
    ...
    $apply_button.='<a class="btn-bg-general tooltip-show btn-block text-white text-center px-5 py-2 text-decoration-none font-weight-bold rounded" onclick="workerstartJob('.$applyJob[0]['user_id'].','.$applyJob[0]['job_id'].')">Accept Job</a>';

}else if(($_SESSION['user_role'] !='home_owner') &&
    ($checkStatus[0]['status']==1) &&
    ( $checkStatus[0]['worker_status']==1) &&
    ($checkStatus[0]['employer_status']==1)){
    $apply_button.='<a class="btn-bg-general tooltip-show btn-block text-white text-center px-5 py-2 text-decoration-none font-weight-bold rounded" onclick="completeJob('.$applyJob[0]['user_id'].','.$applyJob[0]['job_id'].')">Complete</a>';
...
if($imageCount != 0 || $videoCount != 0){
    $jobphotos = '<div class="leads-job-photos-wrapper py-4"><div class="job-photos-heading h3">Job Photos</div>'.$imagesdata.'</div>';
} else { 
    $jobphotos= '';
}
        ...
        <div class="single-lead-desc-first-sec-loc-address d-flex justify-content-between">
            <div>
                ...
                <p class="lead-desc-first-sec-loc font-weight-bold"><i
                        class="fa fa-map"></i>'.$func->getCityName($jobs[0]['post_code'])."
                    ".$func->breakPostalCode($jobs[0]['post_code']).'<span>'. $distanceStr .' </span></p>
            ...
            <div class="single-lead-complete-customer-description">
                ...
                <div class="single-lead-posted-customer-name d-flex justify-content-between">
                    ...
                    <span><a class="text-decoration-none"
                            href="user-profile?u_id='.$user[0]['id'].'&job_id='.$jobs[0]['id'].'&usp=2&ds='.$distance.'&jc='.$countjobs.'">View
